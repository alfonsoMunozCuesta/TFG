...
\texttt{Dash} & Construcción de la interfaz web, navegación entre páginas, actualización dinámica de resultados y conexión entre las acciones del usuario y la lógica interna de la aplicación \cite{dash_tutorial}. \\ \hline
\texttt{Pandas} y \texttt{NumPy} & Carga, limpieza y transformación del dataset, además de preparación de los datos que posteriormente se entregan a los modelos de supervivencia \cite{pandas_manual_alf,numpy_manual_alf}. \\ \hline
...
\texttt{Plotly} & Generación de gráficas interactivas para el dashboard y reutilización de esas mismas visualizaciones en la exportación de informes \cite{plotly_python_intro}. \\ \hline
...
\paragraph*{}
Además, se incorporó una mejora específica de estabilidad para la carga del archivo \texttt{temp\_data.csv}. Al tratarse de un CSV con un volumen considerable de filas y columnas, la interfaz no muestra el archivo completo de golpe, sino una previsualización limitada. Posteriormente, durante el preprocesamiento, el sistema puede recorrer el archivo de forma progresiva para construir directamente el dataset limpio que utilizan los modelos. Con ello se reduce el consumo de memoria y se evita que la aplicación se cierre al trabajar en equipos con recursos limitados.
...
\paragraph*{}
También se unificó la presentación de las respuestas generadas por inteligencia artificial. Todas las técnicas muestran la explicación en un mismo tipo de cuadro de texto, con el mismo tamaño, estilo visual y mensaje de carga. Esta decisión no modifica los resultados estadísticos, pero sí mejora la coherencia de la experiencia de usuario, ya que el funcionamiento del botón ``Explicar'' se percibe igual en Kaplan-Meier, Cox, Log-Rank, Weibull, Exponencial y Random Survival Forest.
...
\paragraph*{}
En la versión final, las interpretaciones no se generan de forma genérica, sino a partir de los resultados activos en pantalla. Según la técnica utilizada, el sistema recopila la tabla, la gráfica, la covariable seleccionada o el perfil simulado vigente, y construye con esa información el contexto que recibe el modelo de lenguaje. Además, se fuerza una redacción orientada a interpretar tendencias y no solo a repetir valores: por ejemplo, se pide explicar la separación de curvas, los descensos pronunciados, los tramos planos, los saltos relevantes y las limitaciones metodológicas de cada análisis.
