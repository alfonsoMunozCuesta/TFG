...
\centering
\small
\begin{tabularx}{\textwidth}{|p{1.1cm}|>{\raggedright\arraybackslash}p{4.9cm}|X|}
\hline
\textbf{Id} & \textbf{Nombre} & \textbf{Descripción} \\ \hline

CU-1 & EjecutarModeloExponencial & 
...
CU-3 & \begin{tabular}[t]{@{}l@{}}EjecutarRandom\\SurvivalForest\end{tabular} & 
...
CU-4 & \begin{tabular}[t]{@{}l@{}}ConsultarComparación\\Técnicas\end{tabular} & 
...
CU-5 & \begin{tabular}[t]{@{}l@{}}ExportarInformes\\YResultados\end{tabular} & 
...
CU-7 & AnalizarVariablesNoBinarias & 
...
CU-9 & \begin{tabular}[t]{@{}l@{}}AmpliarVisualizaciones\\CoxLogRank\end{tabular} & 
...
\paragraph*{}Los casos de uso definidos se apoyan en un conjunto concreto de variables académicas y personales. La selección y justificación de estas variables se desarrolla en el capítulo de diseño, dentro del apartado dedicado al diseño de los datos de análisis, ya que condiciona la estructura interna del sistema y la forma en la que los modelos reciben la información.
...
\begin{itemize}
    ...
    \item La opción de acceso a esta funcionalidad debe estar disponible como botón interno bajo las técnicas de análisis de supervivencia.
\end{itemize} \\ \hline

\textbf{Flujo principal} & 
\begin{enumerate}
    \item El caso de uso comienza cuando el usuario entra en ``Análisis de supervivencia'' y pulsa el botón ``Comparación de técnicas''.
...
\textbf{Descripción} & 
Este caso de uso describe el proceso mediante el cual el sistema genera explicaciones más precisas, coherentes y rápidas mediante una mejora del módulo de inteligencia artificial. La interpretación generada no se basa en una explicación genérica de la técnica, sino en los resultados activos mostrados en pantalla. Para ello, el sistema prepara un contexto con las tablas, métricas, parámetros, p-valores, intervalos de confianza y curvas asociadas al análisis seleccionado, de forma que la respuesta de la IA sea coherente con la gráfica y la tabla visibles en ese momento. \\ \hline
...
\begin{enumerate}
    ...
    \item El sistema recopila la información relevante del análisis ejecutado y de la configuración activa en pantalla.
...
\paragraph*{}A continuación se presentan los diagramas de actividad asociados a los casos de uso de la ampliación. Estos diagramas permiten ver, de manera más dinámica que en la especificación textual, cómo avanza cada funcionalidad desde la acción inicial del usuario hasta la respuesta final del sistema. Además, ayudan a identificar las validaciones, decisiones y posibles caminos alternativos que pueden aparecer durante la interacción con el dashboard.

\subsection{CU-1: EjecutarModeloExponencial}
\paragraph*{}El diagrama del CU-1 representa el proceso que sigue el usuario para ejecutar el modelo exponencial dentro del módulo de análisis de supervivencia. En él se muestra la comprobación inicial del dataset, el cálculo automático del modelo y la posterior visualización de resultados, junto con las acciones complementarias de explicación mediante IA o exportación.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Imagenes/act_cu1_2.png}
    \caption{Diagrama de actividad del CU-1: EjecutarModeloExponencial}
    \label{fig:act_cu1}
\end{figure}

\subsection{CU-2: EjecutarModeloWeibull}
\paragraph*{}El diagrama del CU-2 recoge el flujo de ejecución del modelo Weibull. La actividad comienza cuando el usuario selecciona esta técnica, tras lo cual el sistema verifica que los datos estén preparados, ajusta el modelo y presenta las salidas necesarias para interpretar el comportamiento del riesgo a lo largo del tiempo.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Imagenes/act_cu2_2.png}
...
\subsection{CU-3: EjecutarRandomSurvivalForest}
\paragraph*{}El diagrama del CU-3 describe la ejecución del modelo Random Survival Forest. En este caso, el flujo incorpora tanto el entrenamiento del modelo como la generación de resultados visuales y la simulación de perfiles, lo que permite al usuario explorar escenarios concretos a partir de los valores seleccionados.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Imagenes/act_cu3_2.png}
...
\subsection{CU-4: ConsultarComparacionDeTecnicas}
\paragraph*{}El diagrama del CU-4 muestra el acceso a la página de comparación de técnicas desde la sección de análisis de supervivencia. La verdad es que este flujo resulta útil porque resume cómo el usuario llega a una vista pensada para comparar métodos, revisar sus diferencias principales y apoyarse en una explicación automática cuando está disponible.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Imagenes/act_cu4_2.png}
    \caption{Diagrama de actividad del CU-4: ConsultarComparacionDeTecnicas}
    \label{fig:act_cu4}
\end{figure}

\subsection{CU-5: ExportarInformesYResultados}
\paragraph*{}El diagrama del CU-5 representa el proceso de exportación de informes y resultados. En él se refleja cómo el usuario solicita la generación de un documento o archivo reutilizable, mientras el sistema recopila tablas, gráficas e interpretaciones disponibles antes de construir la salida final.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Imagenes/act_cu5_2.png}
...
\subsection{CU-6: CambiarIdiomaInterfaz}
\paragraph*{}El diagrama del CU-6 describe el cambio de idioma de la interfaz. El flujo es sencillo, pero importante para la experiencia de uso, ya que muestra cómo el usuario selecciona un idioma y cómo el sistema actualiza los textos visibles del dashboard sin alterar los datos ni los resultados ya cargados.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Imagenes/act_cu6_2.png}
    \caption{Diagrama de actividad del CU-6: CambiarIdiomaInterfaz}
    \label{fig:act_cu6}
\end{figure}

\subsection{CU-7: AnalizarVariablesNoBinarias}
\paragraph*{}El diagrama del CU-7 explica cómo se incorporan variables no binarias al análisis. Este proceso amplía el alcance del dashboard, porque permite trabajar con factores que no se reducen a dos categorías y que, en muchos casos, aportan una lectura más rica del abandono académico.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Imagenes/act_cu7_2.png}
    \caption{Diagrama de actividad del CU-7: AnalizarVariablesNoBinarias}
    \label{fig:act_cu7}
\end{figure}

\subsection{CU-8: MejorarInterpretacionIA}
\paragraph*{}El diagrama del CU-8 representa el flujo de mejora de la interpretación mediante inteligencia artificial. Se muestra cómo el sistema recoge los resultados del análisis, prepara el contexto necesario y solicita una explicación al módulo de IA, buscando que la respuesta sea comprensible, coherente y útil para el usuario.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Imagenes/act_cu8_2.png}
    \caption{Diagrama de actividad del CU-8: MejorarInterpretacionIA}
    \label{fig:act_cu8}
\end{figure}

\subsection{CU-9: AmpliarVisualizacionesCoxTestLogRank}
\paragraph*{}El diagrama del CU-9 recoge la generación de visualizaciones adicionales asociadas a la regresión de Cox y al Test de Log-Rank. Su objetivo es mostrar cómo el sistema transforma los resultados estadísticos en salidas gráficas más claras, facilitando la comparación entre grupos y la interpretación visual de las diferencias detectadas.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Imagenes/act_cu9_2.png}
