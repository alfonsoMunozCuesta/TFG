...
\paragraph*{}
El archivo \texttt{cargaDataset.py} actúa como punto de entrada de la aplicación. En él se inicializa el dashboard, se configuran las rutas principales y se gestiona la navegación entre páginas. El archivo \texttt{layout.py} concentra la definición visual de las distintas pantallas, mientras que \texttt{analysis\_callbacks.py} contiene gran parte de la lógica de interacción entre la interfaz y los análisis. Esta separación ayuda a que el diseño sea más comprensible: por un lado está lo que se muestra, y por otro lo que ocurre cuando el usuario interactúa con la aplicación.

\paragraph*{}
Como apoyo a esta estructura, el módulo \texttt{config.py} centraliza rutas y constantes de configuración, pero evita cargar los conjuntos de datos en el momento de importar el módulo. En su lugar, la carga se realiza bajo demanda cuando alguna parte de la aplicación necesita acceder a los ficheros CSV. Esta decisión reduce acoplamientos innecesarios entre módulos y evita que la aplicación falle al arrancar únicamente porque un archivo de datos todavía no esté disponible. Además, el modo de depuración del servidor Dash se plantea como una opción configurable mediante variable de entorno, de forma que el arranque normal del sistema no dependa de un modo de desarrollo activado de forma permanente.

\paragraph*{}
La parte analítica se reparte en módulos específicos según la técnica utilizada. Los modelos ya presentes en el sistema base, como Kaplan-Meier, regresión de Cox y Test de Log-Rank, se complementan con los nuevos módulos \texttt{exponential.py}, \texttt{weibull.py} y \texttt{rsf.py}. De esta forma, cada técnica mantiene su propia lógica de cálculo y puede evolucionar sin afectar directamente al resto del sistema. En el caso concreto de la regresión de Cox, el diseño contempla mecanismos de recuperación ante problemas de convergencia, utilizando regularización antes de recurrir a simplificar el conjunto de covariables. Además, el módulo \texttt{survival\_plots.py} recoge visualizaciones asociadas a los análisis, y los módulos \texttt{pdf\_exporter.py} y \texttt{pdf\_callbacks.py} se encargan de la exportación de resultados.
...
\hline
\textbf{Módulo} & \textbf{Responsabilidad principal} \\ \hline
\texttt{cargaDataset.py} & Inicialización de la aplicación, rutas, navegación y arranque del dashboard. \\ \hline
\texttt{layout.py} & Construcción de las páginas, componentes visuales, formularios y modales. \\ \hline
\texttt{analysis\_callbacks.py} & Gestión de callbacks, interacción del usuario, carga, validación y ejecución de análisis. \\ \hline
\texttt{config.py} & Centralización de rutas y constantes, con carga de datos bajo demanda para reducir dependencias al inicio. \\ \hline
\texttt{preprocesamiento.py} & Limpieza y preparación del conjunto de datos antes de aplicar los modelos. \\ \hline
\texttt{kaplan\_meier.py}, \texttt{cox\_regression.py} y \texttt{log\_rank\_test.py} & Técnicas de análisis de supervivencia procedentes del sistema base. \\ \hline
\texttt{exponential.py}, \texttt{weibull.py} y \texttt{rsf.py} & Nuevas técnicas incorporadas en esta ampliación del dashboard. \\ \hline
\texttt{ollama\_AI.py} & Generación de explicaciones interpretativas mediante inteligencia artificial local. \\ \hline
\texttt{pdf\_exporter.py} y \texttt{pdf\_callbacks.py} & Construcción y descarga de informes PDF con resultados, gráficas e interpretaciones. \\ \hline
\texttt{translations.py} & Gestión de textos en castellano e inglés para el soporte multilingüe. \\ \hline
\end{tabular}
\caption{Distribución modular del sistema}
...
\paragraph*{}
El componente central es la aplicación Dash, inicializada desde \texttt{cargaDataset.py}. A partir de ella se conectan los layouts definidos en \texttt{layout.py} y los callbacks registrados en \texttt{analysis\_callbacks.py} y \texttt{pdf\_callbacks.py}. Los callbacks actúan como punto de unión entre la interacción del usuario y los módulos funcionales. Por ejemplo, cuando el usuario selecciona el modelo de Weibull, el callback correspondiente recoge el dataset preparado, invoca la función de análisis y devuelve a la interfaz los resultados que deben mostrarse.

\paragraph*{}
En cuanto a clases propiamente dichas, destaca \texttt{SurvivalAnalysisPDFExporter}, definida en \texttt{pdf\_exporter.py}. Esta clase concentra la responsabilidad de construir los informes PDF, añadiendo secciones, tablas, gráficas e interpretaciones. Su existencia tiene sentido porque la generación de documentos requiere mantener una estructura interna y reutilizar operaciones comunes en distintos tipos de informe.
